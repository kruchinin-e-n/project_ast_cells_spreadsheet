...
namespace ASTImpl {
	...
	constexpr PrecedenceRule PRECEDENCE_RULES[EP_END][EP_END] = {
		{PR_NONE,  PR_NONE,  PR_NONE,  PR_NONE,  PR_NONE, PR_NONE}, // EP_ADD
		{PR_RIGHT, PR_RIGHT, PR_NONE,  PR_NONE,  PR_NONE, PR_NONE}, // EP_SUB
		{PR_BOTH,  PR_BOTH,  PR_NONE,  PR_NONE,  PR_NONE, PR_NONE}, // EP_MUL
		{PR_BOTH,  PR_BOTH,  PR_RIGHT, PR_RIGHT, PR_NONE, PR_NONE}, // EP_DIV
		{PR_BOTH,  PR_BOTH,  PR_NONE,  PR_NONE,  PR_NONE, PR_NONE}, // EP_UNARY
		{PR_NONE,  PR_NONE,  PR_NONE,  PR_NONE,  PR_NONE, PR_NONE}, // EP_ATOM
	};
	...
	namespace {
		class BinaryOpExpr final : public Expr {
			public:
				...
				double Evaluate(const std::function<double(Position)>& args) const override {
					double result = 0.0;
					switch (type_) {
						case Add:      result = lhs_->Evaluate(args) + rhs_->Evaluate(args); break;		// Evaluate the left-hand side and right-hand side expressions and perform addition
						case Subtract: result = lhs_->Evaluate(args) - rhs_->Evaluate(args); break;		// Evaluate the left-hand side and right-hand side expressions and perform subtraction
						case Multiply: result = lhs_->Evaluate(args) * rhs_->Evaluate(args); break;		// Evaluate the left-hand side and right-hand side expressions and perform multiplication
						case Divide:   result = lhs_->Evaluate(args) / rhs_->Evaluate(args); break;		// Evaluate the left-hand side and right-hand side expressions and perform division
					}
					if (!std::isfinite(result)) {		// Check if the result is finite
						throw FormulaError{ FormulaError::Category::Div0 };		// If the result is not finite throw a FormulaError
					}
		...
		public:
			...
			double Evaluate(const std::function<double(Position)>& args) const override {
				switch (type_) {
					case UnaryMinus: return -(operand_->Evaluate(args));	// Evaluate the operand and return its negation
					case UnaryPlus: return operand_->Evaluate(args);		// Evaluate the operand and return ist positive value
					default: throw std::logic_error("Error: unexpected unary operator type");		// Handle unexpected cases by throwing an exception or returning a default value
				}
			}
		...
		public:
			...
			void Print(std::ostream& output) const override {
				if (!cell_->IsValid()) { output << FormulaError::Category::Ref; }
				else { output << cell_->ToString(); }
			}

			void DoPrintFormula(std::ostream& output, ExprPrecedence precedence) const override { Print(output); }
		...
		public:
			...
			void Print(std::ostream& output) const override { output << value_; }

			void DoPrintFormula(std::ostream& out, ExprPrecedence precedence) const override { out << value_; }

			ExprPrecedence GetPrecedence() const override { return EP_ATOM; }

			double Evaluate(const std::function<double(Position)>& args) const override { return value_; }
...
void FormulaAST::PrintCells(std::ostream& out) const {
	for (auto cell : cells_) { out << cell.ToString() << ' '; }
}
...
double FormulaAST::Execute(const std::function<double(Position)>& args) const {	return root_expr_->Evaluate(args); }
