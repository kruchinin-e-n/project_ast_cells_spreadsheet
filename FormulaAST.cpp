...
enum PrecedenceRule {
  // never needed
  PR_NONE = 0b00,
  // needed for a left child
  PR_LEFT = 0b01,
  // needed for a right child
  PR_RIGHT = 0b10,
  // needed for both children
...
//
// The interesting cases are the ones where removing
// the parens would change the AST.
// It may happen when our precedence rules for parentheses are different from
// the grammatic precedence of operations.
//
// Case analysis:
// A + (B + C) - always okay (nothing of lower grammatic precedence could
...
// A + (B - C) - always okay (nothing of lower grammatic precedence could
// have been written to the
// right) A - (B + C) - never okay A - (B - C) - never okay A * (B * C)
// - always okay (the parent
// has the highest grammatic precedence) A * (B / C) - always okay
// (the parent has the highest
// grammatic precedence) A / (B * C) - never okay A / (B / C) - never okay
// -(A + B) - never okay
// -(A - B) - never okay
// -(A * B) - always okay (the resulting binary op has
// the highest grammatic precedence)
// -(A / B) - always okay (the resulting binary op has
// the highest grammatic precedence)
// +(A + B) - **sometimes okay** (e.g. parens in +(A + B) / C
// are **not** optional)
...
class Expr {
  ...
  virtual void DoPrintFormula(
      std::ostream& out, ExprPrecedence precedence) const = 0;
  virtual double Evaluate(
      const std::function<double(Position)>& args) const = 0;
...
class BinaryOpExpr final : public Expr {
  ...
  double Evaluate(const std::function<double(Position)>& args) const override {
    ...
    switch (type_) {
      case Add:
        // Evaluate the left-hand side and right-hand
        // side expressions and perform addition
        result = lhs_->Evaluate(args) + rhs_->Evaluate(args);
        break;
      case Subtract:
        // Evaluate the left-hand side and right-hand
        // side expressions and perform subtraction
        result = lhs_->Evaluate(args) - rhs_->Evaluate(args);
        break;
      case Multiply:
        // Evaluate the left-hand side and right-hand
        // side expressions and perform multiplication
        result = lhs_->Evaluate(args) * rhs_->Evaluate(args);
        break;
      case Divide:
        // Evaluate the left-hand side and right-hand
        // side expressions and perform division
        result = lhs_->Evaluate(args) / rhs_->Evaluate(args);
        break;
    }
    // Check if the result is finite
    if (!std::isfinite(result)) {
      // If the result is not finite throw a FormulaError
...
class UnaryOpExpr final : public Expr {
  ...
  double Evaluate(const std::function<double(Position)>& args) const override {
    switch (type_) {
      ...
      case UnaryPlus:
        // Evaluate the operand and return ist positive value
        return operand_->Evaluate(args);
      default:
        // Handle unexpected cases by throwing
        // an exception or returning a default value
        throw std::logic_error("Error: unexpected unary operator type");
    }
  }
...
class CellExpr final : public Expr {
  ...
  void DoPrintFormula(std::ostream& output,
                      ExprPrecedence precedence) const override {
    Print(output);
  }
...
class ParseASTListener final : public FormulaBaseListener {
  ...
  void exitUnaryOp(FormulaParser::UnaryOpContext* ctx) override {
    ...
    if (ctx->SUB()) { type = UnaryOpExpr::UnaryMinus; }
    else {
