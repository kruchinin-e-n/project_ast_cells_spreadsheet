...
void TestPositionAndStringConversion() {
  ...
  testSingle(Position{ Position::MAX_ROWS - 1,
                       Position::MAX_COLS - 1 }, "XFD16384");
}
  ...
  auto evaluate =
      [&](std::string expr) {
        return std::get<double>(
            ParseFormula(std::move(expr))->Evaluate(*sheet));
      };
...
void TestFormulaInvalidPosition() {
  ...
  auto try_formula = [&](const std::string& formula) {
    ...
    catch (const FormulaException&) { /* we expect this one */ }
  };
...
void TestFormulaIncorrect() {
  auto isIncorrect =
    [](std::string expression) {
      try { ParseFormula(std::move(expression)); }
      catch (const FormulaException&) { return true; }
      return false;
    };
...
void TestCellCircularReferences() {
  ...
  bool caught = false;
  try { sheet->SetCell("M6"_pos, "=E2"); }
  catch (const CircularDependencyException&) { caught = true; }
...
int main() {
  ...
  sheet->SetCell("A1"_pos, "=1*1");
  sheet->SetCell("A2"_pos, "=1*2");
  sheet->SetCell("A3"_pos, "=1*3");
  sheet->SetCell("B1"_pos, "=2*1");
  sheet->SetCell("B2"_pos, "=2*2");
  sheet->SetCell("B3"_pos, "=2*3");
  sheet->SetCell("C1"_pos, "=3*1");
  sheet->SetCell("C2"_pos, "=3*2");
  sheet->SetCell("C3"_pos, "=3*3");
